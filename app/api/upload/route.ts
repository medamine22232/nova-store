...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadPreset = process.env.CLOUDINARY_UPLOAD_PRESET; // optional (for unsigned uploads)
    ...
    if (cloudName && (uploadPreset || (apiKey && apiSecret))) {
      ...
      if (uploadPreset) {
        ...
        // signature = sha1 of `timestamp=${timestamp}${apiSecret}`
    ...
    return NextResponse.json(
      { error: 'Failed to upload file', details: error instanceof Error ? error.message : 'Unknown' },
      { status: 500 }
    );
  }
}